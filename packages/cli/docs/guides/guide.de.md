...
`locked` steht für die jeweilige lockfile/frozen Installation des Managers.
Intern nutzt OpenDock `npm ci`, `pnpm install --frozen-lockfile`,
`bun install --frozen-lockfile` oder `uv sync --frozen`.
