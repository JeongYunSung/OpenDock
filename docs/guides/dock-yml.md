...
`dock.yml`은 OpenDock starterpack의 진입점입니다. 어떤 파일을 프로젝트에 넣을지, 기존 파일을 어떻게 보존할지, 설치와 업데이트 때 어떤 명령을 실행할지, doctor에서 어떤 상태를 점검할지를 선언합니다.

이 문서는 현재 OpenDock CLI 구현 기준입니다. 미래에 추가될 수 있는 marketplace 기능이나 DockHub 서버 정책이 아니라, 지금 `src/pack.ts`, `src/installer.ts`, `src/runner.ts`가 실제로 해석하는 형식을 설명합니다.

실제 예제 pack은 `examples/` 아래에 역할별로 나뉘어 있습니다.

| 예제 | 용도 |
|---|---|
| `examples/git/dock.yml` | Git 설치와 프로젝트 초기화를 담당하는 기본 pack |
| `examples/codex/dock.yml` | Codex CLI만 설치하는 기본 pack |
| `examples/claude-code/dock.yml` | Claude Code를 설치하는 pack |
| `examples/oh-my-codex/dock.yml` | Codex CLI와 Oh My Codex를 설치하는 pack |
| `examples/oh-my-openagent/dock.yml` | Codex CLI와 Oh My OpenAgent Codex Light를 설치하는 pack |

## 기본 구조

starterpack은 보통 다음 구조를 가집니다.
...
권장 형식은 `opendock: 1`입니다. 과거 호환용으로 `schema: opendock/v1`, `kind: starterpack`, `setup`도 파싱되지만 새 starterpack은 `opendock`, `files`, `lifecycle` 중심으로 작성하는 편이 좋습니다.
...
| `id` | 필수 | pack 식별자입니다. 설치 요청의 `owner/name`과 정확히 같아야 합니다. |
| `name` | 선택 | 사람이 읽는 이름입니다. 현재 실행 로직에는 영향이 없습니다. |
| `summary` | 선택 | 설명 문자열입니다. 기본값은 빈 문자열입니다. |
| `version` | 권장 필수 | pack 버전입니다. 없으면 `0.1.0`으로 해석됩니다. |
...
`files`는 starterpack 안의 파일을 프로젝트 루트로 적용하는 목록입니다.
...
| 필드 | 필수 | 설명 |
|---|---:|---|
| `from` | 필수 | pack root 기준 소스 파일 경로입니다. |
...
`files`를 비워두고 `templates/`만 둔 과거 형식도 동작합니다. 이 경우 `.gitignore`는 `append_unique`, 나머지 파일은 기존 파일이 있으면 managed block 방식에 가깝게 처리됩니다. 하지만 새 pack은 파일별 의도가 분명한 `files`를 쓰는 것이 좋습니다.
...
| `update` | `opendock update` | 이미 설치된 pack을 최신 버전 또는 최신 상태로 갱신 |
...
명령은 shell expansion을 기대하지 않는 방식으로 써야 합니다. `~`, `$HOME`, glob, redirect, pipe가 필요한 복잡한 작업은 현재 MVP에서 직접 지원하지 않습니다. 그런 작업이 필요하면 starterpack 자체의 파일 적용이나 allowlist에 있는 도구의 단일 명령으로 표현할 수 있는지 먼저 확인하세요.
...
`interactive: scripted`는 macOS의 `expect`를 사용해 pseudo-terminal에서 키 입력을 보냅니다. 승인된 DockHub pack에서만 신중하게 쓰는 것이 좋습니다.
...
`install`은 최초 적용입니다. 없는 도구를 설치하고 프로젝트에 starterpack을 적용하는 데 집중하세요.
...
파일 업데이트는 `files[].update` 정책에 따라 `update`에서도 다시 적용됩니다. 예를 들어 `managed_block` 파일은 OpenDock block이 새 pack 내용으로 교체되고, `append_unique`는 새 라인만 추가됩니다.

## 기존 프로젝트 파일을 보호하는 기준

starterpack은 사용자가 이미 작성한 파일을 함부로 덮어쓰면 안 됩니다.
...
`managed_block` 안의 내용은 OpenDock이 업데이트하는 영역으로 보는 것이 맞습니다. 사용자가 직접 고칠 수는 있지만, 다음 update에서 pack 내용으로 바뀔 수 있습니다. 사용자 고유 내용은 block 바깥에 쓰는 방식이 안전합니다.

## 보안과 배포 기준

OpenDock은 프로젝트 설정을 실행하는 도구이므로 pack source를 런타임 환경변수로 바꿀 수 없게 고정합니다. 현재 registry는 `https://opencode.app`입니다.

배포 흐름은 다음 원칙을 따릅니다.

1. pack author가 `dock.yml`과 `templates/`를 작성합니다.
2. `opendock auth login`으로 로그인합니다.
3. `opendock deploy <pack-name>`으로 제출합니다.
4. DockHub/OpenDock 측 검토를 통과한 pack만 registry에서 설치될 수 있습니다.

`install`은 공개 명령이지만, 설치 대상 pack은 승인된 registry metadata, signature, checksum 검증을 통과해야 합니다.
...
처음부터 큰 pack을 만들기보다 아래 순서로 작성하면 실수가 줄어듭니다.
...
7. pack 제출 전에 사용자 파일이 보존되는지, 재실행해도 중복 block이 생기지 않는지 확인한다.
...
Oh My OpenAgent는 OpenCode용 Ultimate와 Codex용 Light 구성이 나뉩니다. Codex 설치 starterpack에서는 `--platform=codex`를 명시하세요. 공식 문서의 Codex-only 예시는 `npx lazycodex-ai install --no-tui --codex-autonomous`도 안내합니다. OpenDock에서는 둘 중 하나를 pack 정책으로 정해 일관되게 쓰면 됩니다.
