...
describe("example dock cleanup behavior", () => {
  ...
  it("cleans exported task outputs from the oma example", async () => {
    ...
    expect(
      readFileSync(join(project, ".claude", "skills", "architecture", "SKILL.md"), "utf8"),
    ).toContain("# Architecture Workflow");
    expect(
      readFileSync(join(project, ".claude", "skills", "oma-brainstorm", "SKILL.md"), "utf8"),
    ).toContain("# Skill");
...
case "$*" in
  *install*)
    ...
    mkdir -p .agents/skills/oma-brainstorm .agents/workflows .agents/cache .codex/agents .codex/skills/oma-brainstorm .claude/skills/architecture .github/instructions
    printf '# OMA Agent\\n\\nGenerated by fake OMA.\\n' > AGENTS.md
    printf '# Claude\\n' > CLAUDE.md
    printf '# Gemini\\n' > GEMINI.md
    printf '# Skill\\n' > .agents/skills/oma-brainstorm/SKILL.md
    printf '# Architecture Workflow\\n' > .agents/workflows/architecture.md
    ln -s ../../.agents/skills/oma-brainstorm .claude/skills/oma-brainstorm
    ln -s ../../../.agents/workflows/architecture.md .claude/skills/architecture/SKILL.md
