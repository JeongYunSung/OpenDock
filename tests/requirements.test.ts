...
describe("requires regression coverage", () => {
  ...
  it("passes BUN_INSTALL to Bun package installers and verification", async () => {
    const project = tempDir();
    const bin = tempDir();
    const home = tempDir();
    const customBunInstall = join(tempDir(), "custom-bun");
    const log = join(project, "commands.log");
    writeFakeBunUsingEnvironment(bin, log);

    const result = await withEnv(
      { BUN_INSTALL: customBunInstall, HOME: home, PATH: bin },
      async () =>
    ...
    expect(
      readFileSync(
        join(customBunInstall, "install", "global", "node_modules", "oh-my-agent", "package.json"),
        "utf8",
      ),
    ).toContain('"version":"8.43.0"');
    expect(readFileSync(log, "utf8")).toContain(`bun-install:${customBunInstall}`);
  });
...
function writeFakeBunUsingEnvironment(bin: string, log: string): void {
  writeExecutable(
    join(bin, "bun"),
    `#!/bin/sh
set -eu
...
if [ "$1" = "install" ]; then
  ...
  /bin/mkdir -p "$bun_root/install/global/node_modules/oh-my-agent"
  /bin/cat > "$bun_root/install/global/node_modules/oh-my-agent/package.json" <<'EOF'
{"name":"oh-my-agent","version":"8.43.0"}
EOF
  exit 0
fi
